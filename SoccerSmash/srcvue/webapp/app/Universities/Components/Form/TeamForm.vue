<template>
  <div class="bg-gray-200" v-if="team && renderForm">
    <div>
      <form id="form" class=" mx-auto bg-white shadow rounded" method="post" action="/universities" @submit="validateForm" enctype="multipart/form-data">
        <div class="xl:w-full border-b border-gray-300 dark:border-gray-700 py-5">
          <div class="flex items-center w-11/12 mx-auto">
            <p class="text-lg text-gray-800 dark:text-gray-100 font-bold">University's Information</p>
            ...
            <div class="my-8 mx-auto xl:w-full xl:mx-0">
              <div class="xl:flex lg:flex md:flex flex-wrap justify-between">
                <div class="xl:w-2/5 lg:w-2/5 md:w-2/5 flex flex-col mb-6">
                  <label for="TeamsName" class="pb-2 text-sm font-bold text-gray-800 dark:text-gray-100">University's Name</label>
        ...
        <div class="w-full py-4 sm:px-12 px-4 bg-gray-100 dark:bg-gray-700 mt-6 flex justify-end rounded-bl rounded-br">
          <div class="btn text-sm focus:outline-none text-gray-600 dark:text-gray-400 border border-gray-300 dark:border-gray-500 py-2 px-6 mr-4 rounded hover:bg-gray-200 transition duration-150 ease-in-out" @click="reset()">Restore</div>
...
<script src="./TeamForm.ts" lang="ts"></script>